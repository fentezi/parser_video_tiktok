...
def AWSsignature(access_key, secret_key, request_parameters, headers, method="GET", payload='', region="us-east-1",
                 service="vod"):
...
def uploadVideo(session_id, video, title, tags, schedule_time=0, verbose=True):
    ...
    for tag in tags:
        ...
        try:
            ...
            verified_tag = tag
        text += " #" + verified_tag
        text_extra.append({"start": len(text) - len(verified_tag) - 1, "end": len(text), "user_id": "", "type": 1,
